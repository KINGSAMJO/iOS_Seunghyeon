...
class InsertRepositoryListUseCase @Inject constructor(
    private val repository: RepositoryRepository,
    @IoDispatcher private val coroutineDispatcher: CoroutineDispatcher
) {
    suspend operator fun invoke(repositoryList: List<RepositoryInformation>): List<Long> {
        return withContext(coroutineDispatcher) {
            repository.insertRepositoryList(repositoryList)
        }
    }
}